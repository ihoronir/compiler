...
static Token new_token_unchecked(TokenKind kind, int line, int row) {
...
Token new_token(TokenKind kind, int line, int row) {
    ...
    Token token = new_token_unchecked(kind, line, row);
    return token;
}

Token new_token_const_int(int val_int, int line, int row) {
    Token token = new_token_unchecked(TK_CONST_INT, line, row);
    token->val_int = val_int;
    return token;
}

Token new_token_ident(char *str, int line, int row) {
    Token token = new_token_unchecked(TK_IDENT, line, row);
    token->str = str;
    return token;
}
